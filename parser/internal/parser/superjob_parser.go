...
func (p *SuperJobParser) SearchVacancies(params interfaces.SearchParams) ([]model.Vacancy, error) {
	...
	return p.convertToVacancies(sjResponse.Objects), nil
}
...
func (p *SuperJobParser) convertToVacancies(sjVacancies []model.SuperJobVacancy) []model.Vacancy {
	vacancies := make([]model.Vacancy, len(sjVacancies))
	for i, sjv := range sjVacancies {
		vacancies[i] = model.Vacancy{
...
func (p *SuperJobParser) GetVacancyByID(vacancyID string) (*model.Vacancy, error) {
	// Реализация получения деталей вакансии по ID
	// Аналогично HH Parser
	return nil, nil
}

func (p *SuperJobParser) SimpleSearch(query string, limit int) ([]model.Vacancy, error) {
